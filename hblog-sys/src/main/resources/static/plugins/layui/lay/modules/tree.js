layui.define("jquery", function(e) {
			...
			i.prototype.init = function(e) {
							...
							s = o(
									[
										...
										function() {
											...
											var eleStr = i.check&&i.check=="checkbox" ? '<input type="checkbox" name="'+n.name+'" '+((n.checked&&n.checked) ? 'checked="checked"' : "")+(n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.parentId +'"' + 'data-role-id="' + n.roleId +'"' + 'id="' + n.id + '"' +(i.checkboxStyle ? (' style="' + i.checkboxStyle + '"') : "" ) :  '';
											if(eleStr.length > 0){
												//给元素添加data-xxx属性
				...
				i.prototype.check = function(ids){
				...
				i.prototype.uncheck = function(ids){
					var $elem = o(this.options.elem),that = this;
					for(var i = 0; i < ids.length; i++){
						var $input = $elem.find("input[data-id='"+ids[i]+"']")[0];
				...
				i.prototype.uncheckAll = function(){
					var $elem = o(this.options.elem),that = this;
					$elem.find("input[type='checkbox']").each(function(i, e){
						that.inputChecked(e,false);
					})
				},
				i.prototype.checkAll = function(){
					var $elem = o(this.options.elem),that = this;
					$elem.find("input[type='checkbox']").each(function(i, e){
						that.inputChecked(e,true);
					})
				},
				i.prototype.disabledAll = function(){
					var $elem = o(this.options.elem),that = this;
					$elem.find("input[type='checkbox']").each(function(i, e){
						e && (e.disabled = "disabled");
					})
				},
				i.prototype.reload = function(options){
					var that = this;
					that.options.nodes = options;
					var $elem = o(that.options.elem);
					$elem.empty();
					that.init($elem);
				...
				e("tree", function(e) {
					...
					t[0] ?  r.init(t) : a.error("layui.tree 没有找到" + e.elem + "元素");
					return r;
				})
})