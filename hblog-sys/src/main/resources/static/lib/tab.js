...
layui.define(['element'], function (exports) {
    ...
    Tab.prototype.init = function () {
        ...
        if (typeof(_config.elem) !== 'string' && typeof(_config.elem) !== 'object') {
            throw new Error('Tab error: elem参数未定义或设置出错，具体设置格式请参考文档API.');
        }
        var $container;
        ...
        if ($container.length === 0) {
            throw new Error('Tab error:找不到elem参数配置的容器，请检查.');
        }
        var filter = $container.attr('lay-filter');
        if (filter === undefined || filter === '') {
            throw new Error('Tab error:请为elem容器设置一个lay-filter过滤器');
        }
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            if (_config.autoRefresh) {
                ...
                //_config.elem.find('div.layui-tab-content > div').eq(tabIndex).children('.iframe').html(getHtml(data.href));
            }
        }
        if (_config.contextMenu) {
...
//									var src = ELEM.contentBox.find('.iframe[data-id=' + id + ']')[0].src;
//									ELEM.contentBox.find('.iframe[data-id=' + id + ']').html(getHtml(src));
                                    break;
