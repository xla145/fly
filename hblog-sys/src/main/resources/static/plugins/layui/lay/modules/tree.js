layui.define("jquery", function (exports) {
    ...
    var $ = layui.jquery,
        hint = layui.hint(),
        layuiTreeEnter = "layui-tree-enter",
        tree = {},
    ...
    Tree.prototype.init = function () {
        ...
        let elem = that.options.elem;

        let $elem = $(elem);

        $elem[0] ? $elem.addClass("layui-box layui-tree") : hint.error("layui.tree 没有找到" + elem + "元素");

        that.options.skin && $elem.addClass("layui-tree-skin-" + that.options.skin),
            that.tree($elem),
            that.on($elem)
    };


    Tree.prototype.tree = function (e, node) {//生成树
        let that = this,
            options = that.options,
            nodes = node || options.nodes;


        layui.each(nodes, function (i, n) {
            let id = that.uuid();

            options.id = id;//给每一个节点都设置id

            if (nodes.children) {//如果当前节点有子节点，则将子节点的pid设置成当前节点的id
                layui.each(nodes.children, function (index, item) {
                    item.pid = nodes.id;
                });
            }
            ...
            var l = n.children && n.children.length > 0,
                ...
                //生成ul
                c = $('<ul class="' + (n.spread ? "layui-show" : "") + '"></ul>'),
                //生成节点
                s = $(["<li " + (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">",
                        function () {
                            return l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + '</i>' : '<i style="margin-left: 24px;"></i>';
                        }(),
                        function () {

                            //如果用户选择的风格是checkbox则生成checkbox
                            // console.log(n.checked)
                            var eleStr = options.check && options.check == "checkbox" ? '<input type="checkbox" name="' + n.name + '" ' + ((n.checked && n.checked) ? 'checked="checked"' : "") + (n.checkboxValue ? ('value="' + n.checkboxValue + '"') : "") + 'data-parent-id="' + n.parentId + '"' + 'data-role-id="' + n.roleId + '"' + 'id="' + n.id + '"' + (options.checkboxStyle ? (' style="' + options.checkboxStyle + '"') : "") : '';
                            if (eleStr.length > 0) {
                                //给元素添加data-xxx属性
                        ...
                        function () {
                            //a链接是否可点击
                            return '<a href="' + (n.href || "javascript:;") + '" ' + (options.target && n.href ? 'target="' + options.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a>")
                        }(),
                        "</li>"
                    ].join("")
                );
            l && (s.append(c), that.tree(c, n.children)),
                e.append(s),
            "function" == typeof options.click && that.click(s, n),
                that.spread(s, n),
            options.drag && that.drag(s, n)
            that.changed(s, n)
        })
    };


    Tree.prototype.changed = function (e, o) {
        //当checkbox的值改变后所处理动作
        var that = this;
    ...
    Tree.prototype.cancelParentsCheckboxCheck = function (ele) {
    ...
    Tree.prototype.siblingInputs = function (ele) {
    ...
    Tree.prototype.currentChildrenInputs = function (ele) {
    ...
    Tree.prototype.inputChecked = function (ele, checked) {
        //checkbox选中或不选中
    ...
    Tree.prototype.parentsChecked = function (e, checked) {
    ...
    Tree.prototype.findParents = function (ele, selector) {
        //查找元素的指定父元素
        var parent = ele.parentElement,
            that = this;
    ...
    Tree.prototype.click = function (e, o) {
        var a = this,
            r = a.options;
    ...
    Tree.prototype.spread = function (e, o) {
        //这个方法的功能是展开子节点
        var a = this,
            //r就是小箭头
    ...
    Tree.prototype.move = {};
    Tree.prototype.drag = function (e, a) {
        var i = this,
    ...
    Tree.prototype.check = function (ids) {
        var $elem = o(this.options.elem), that = this;
        for (var i = 0; i < ids.length; i++) {
            var $input = $elem.find("input[data-id='" + ids[i] + "']")[0];
            that.inputChecked($input, true);
            that.parentsChecked($input, true);
        }
    };
    Tree.prototype.uncheck = function (ids) {
        var $elem = o(this.options.elem), that = this;
        for (var i = 0; i < ids.length; i++) {
            var $input = $elem.find("input[data-id='" + ids[i] + "']")[0];
            that.inputChecked($input, false);
            that.cancelParentsCheckboxCheck($input);
        }
    };
    Tree.prototype.uncheckAll = function () {
        var $elem = o(this.options.elem), that = this;
        $elem.find("input[type='checkbox']").each(function (i, e) {
            that.inputChecked(e, false);
        })
    };
    ...
    Tree.prototype.checkAll = function () {
        var $elem = o(this.options.elem), that = this;
        $elem.find("input[type='checkbox']").each(function (i, e) {
            that.inputChecked(e, true);
        })
    };


    Tree.prototype.disabledAll = function () {
        var $elem = o(this.options.elem), that = this;
        $elem.find("input[type='checkbox']").each(function (i, e) {
     ...
     * @param nodes
     */
    Tree.prototype.reload = function (nodes) {
        let that = this;
        that.options.nodes = nodes;
        that.init();
    }


    tree.render = function (options) {
        let t = new Tree(options);
        t.init()
        return t;
    }
