...
layui.define('layer', function(exports){
    ...
    //表单序列化为json
    Form.prototype.serializeJson = function(dom){
        var jsonData = {};
                    ...
                    dds.on('click', function(event){
                        ...
                        if(typeof select.attr('multiple') && typeof select.attr('multiple') === 'string'){
                            ...
                            if(othis.find("input[type='checkbox']").is(':checked')){
                                multiSelect.html(multiSelect.html() + "<a href='javascript:;'><span>"+othis.find("span").text()+"</span><i class='layui-icon'>✖</i></a>");
                                valueStr.push(value);
                            }else{
                            ...
                            , '<div class="' + TITLE + '"><input type="text" class="layui-input" placeholder="' + placeholder + '"><div class="layui-input multiSelect" >' + function(){
                                ...
                                if(value != null && value != undefined && value.length != 0){
                                    for(var aList = 0;aList<value.length;aList++){
                                        if(value[aList]){
                                            aLists.push("<a href='javascript:;'><span>"+select[value[aList]].text+"</span><i class='layui-icon'>✖</i></a>")
                                        }
                                    }
                                }
                                return aLists.join('');
    ...
    //校验表单
    Form.prototype.onVerify = function($form){
        var verify = form.config.verify, stop = false
            ,DANGER = 'layui-form-danger', field = {} ,elem = $form
            ,verifyElem = elem.find('*[lay-verify]') //获取需要校验的元素
            ,formElem = $form[0] //获取当前所在的form元素，如果存在的话
